...
- [x] Smarter quote/brace autocomplete (no doubled quotes); "fix broken pairs"
      ...
      caret abuts a word char; format-on-save pref (silent no-op for
      unsupported types); Format Selection = clang-format --lines over the
      selected range, Ctrl+K Ctrl+F + menu + palette; broken-pairs are fixed by
      the formatter pass itself)
