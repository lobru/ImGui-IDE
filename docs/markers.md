...
Markers are associated with the specified lines so when lines are inserted or deleted above, the marker will move as well. When the marked line is deleted or all markers are removed via the API, the marker disappears.
