...
static const std::vector<std::string>* compiledMembers(const std::string& simpleType)
{
	...
	static const std::unordered_map<std::string, std::vector<std::string>> table = {
		...
		// ── Unreal Engine containers / core types ────────────────────────────
		// Same mechanism as the STL entries: a declared "TMap<FString,FString> m;"
		// reduces to the simple name "TMap", so "m." completes these. Name-only,
		// curated to the common interface (not exhaustive).
		{"TArray", {"Add", "AddUnique", "AddZeroed", "AddDefaulted", "Emplace", "EmplaceAt",
			"Append", "Insert", "Init", "Push", "Pop", "Top", "Last", "Remove", "RemoveAt",
			"RemoveAtSwap", "RemoveSingle", "RemoveSwap", "RemoveAll", "Empty", "Reset",
			"SetNum", "SetNumZeroed", "SetNumUninitialized", "Reserve", "Shrink", "Num",
			"Max", "GetData", "GetTypeSize", "IsEmpty", "IsValidIndex", "Contains",
			"ContainsByPredicate", "Find", "FindLast", "FindByKey", "FindByPredicate",
			"IndexOfByKey", "IndexOfByPredicate", "Sort", "StableSort", "HeapSort",
			"HeapPush", "HeapPop", "Swap", "SwapMemory"}},
		{"TMap", {"Add", "Emplace", "FindOrAdd", "FindRef", "FindChecked", "Find", "Contains",
			"Remove", "Empty", "Reset", "Num", "IsEmpty", "GetKeys", "GenerateKeyArray",
			"GenerateValueArray", "KeySort", "ValueSort", "Reserve", "Shrink", "Append",
			"OrderIndependentCompareEqual"}},
		{"TSet", {"Add", "Emplace", "Contains", "Remove", "Find", "FindId", "Num", "Empty",
			"Reset", "IsEmpty", "Reserve", "Shrink", "Sort", "Array", "Intersect", "Union",
			"Difference", "Includes", "CountByPredicate"}},
		{"TSharedPtr", {"Get", "Reset", "IsValid", "ToSharedRef", "GetSharedReferenceCount",
			"IsUnique", "operator bool"}},
		{"TSharedRef", {"Get", "ToSharedPtr", "GetSharedReferenceCount"}},
		{"TWeakPtr", {"Pin", "IsValid", "Reset", "GetSharedReferenceCount"}},
		{"TWeakObjectPtr", {"Get", "IsValid", "IsStale", "Reset"}},
		{"TUniquePtr", {"Get", "Reset", "Release", "IsValid", "operator bool"}},
		{"TObjectPtr", {"Get", "IsNull", "IsValid", "Reset"}},
		{"TOptional", {"GetValue", "Get", "IsSet", "Reset", "Emplace"}},
		{"FString", {"Len", "IsEmpty", "Append", "Appendf", "AppendChar", "AppendChars",
			"Reset", "Empty", "Contains", "Equals", "Compare", "Find", "Replace",
			"ReplaceInline", "ReplaceCharWithEscapedChar", "Split", "Left", "LeftInline",
			"Right", "RightInline", "Mid", "MidInline", "LeftChop", "RightChop",
			"TrimStartAndEnd", "TrimStart", "TrimEnd", "TrimQuotes", "ToUpper", "ToLower",
			"StartsWith", "EndsWith", "MatchesWildcard", "ParseIntoArray", "ParseIntoArrayLines",
			"ParseIntoArrayWS", "IsNumeric", "ToBool", "Reverse", "ReverseString",
			"RemoveFromStart", "RemoveFromEnd", "GetCharArray"}},
		{"FName", {"ToString", "IsValid", "IsNone", "GetStringLength", "GetPlainNameString",
			"Compare", "IsEqual", "GetComparisonIndex", "GetNumber"}},
		{"FText", {"ToString", "IsEmpty", "IsEmptyOrWhitespace", "EqualTo", "EqualToCaseIgnored",
			"CompareTo", "CompareToCaseIgnored", "IsNumeric", "ToUpper", "ToLower"}},
		{"FVector", {"Size", "SizeSquared", "Length", "SquaredLength", "Normalize",
			"GetSafeNormal", "GetUnsafeNormal", "IsNearlyZero", "IsZero", "IsNormalized",
			"Dist", "DistSquared", "DotProduct", "CrossProduct", "ToString", "GridSnap",
			"ComponentMax", "ComponentMin", "GetAbs", "GetMax", "GetMin", "X", "Y", "Z"}},
		{"FVector2D", {"Size", "SizeSquared", "Normalize", "GetSafeNormal", "IsNearlyZero",
			"IsZero", "Dot", "ToString", "X", "Y"}},
		{"FRotator", {"Vector", "Euler", "Quaternion", "GetNormalized", "Normalize",
			"IsNearlyZero", "IsZero", "Add", "Clamp", "ToString", "Pitch", "Yaw", "Roll"}},
		{"FQuat", {"Rotator", "Euler", "GetNormalized", "Normalize", "IsNormalized",
			"RotateVector", "UnrotateVector", "GetAxisX", "GetAxisY", "GetAxisZ", "ToString"}},
		{"FTransform", {"GetLocation", "GetRotation", "GetScale3D", "GetTranslation",
			"SetLocation", "SetRotation", "SetScale3D", "TransformPosition",
			"TransformVector", "InverseTransformPosition", "Inverse", "ToString"}},
	};
