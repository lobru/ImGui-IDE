...
void Editor::openPdfFile(const std::string &path)
{
    for (auto &p : pdfs)
    {
        if (p->path == path)
...
void Editor::renderPdfWindows()
{
    ...
    for (auto it = pdfs.begin(); it != pdfs.end();)
    {
        ...
        if (ImGui::Begin(p.windowTitle.c_str(), &p.open))
        {
            ...
            middleMousePanScroll(11); // pdf viewport — honors the pan-invert setting
            ImGui::EndChild();
        }
        ImGui::End();
        ++it;
...
void Editor::openFile(const std::string &path)
{
    ...
    // fall through to the editor below.
    {
        ...
        if (ext == ".pdf") // in-app viewer (OS PDF engine) — must beat isBinaryExt
        {
            openPdfFile(path);
            return;
        }
